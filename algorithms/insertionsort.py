...
Complete visualization of the Insertion Sort algorithm with step-by-step animation.
"""
...
class InsertionSortAlgorithm(SortingAlgorithmBase):
    """
    Insertion Sort algorithm visualization implementation
    """
    
    # Algorithm metadata
    algorithm_name = "Insertion Sort"
    description = """Insertion Sort is a simple sorting algorithm that builds the final sorted array 
    one element at a time. It works by taking elements from the unsorted portion and inserting 
    them into their correct position in the sorted portion. It is similar to how people sort 
    playing cards in their hands - you pick up cards one by one and insert each into its 
    proper position among the cards you've already sorted."""
    time_complexity = "O(n²)"
    ...
    def __init__(self, parent_frame):
        """Initialize Insertion Sort visualization"""
        self.colors = self.get_color_scheme()
        self.start_time = None
        super().__init__(parent_frame)
    
    def create_gui(self):
        """Create the Insertion Sort specific GUI components"""
        ...
        title_label = ttk.Label(title_frame, text="Insertion Sort Algorithm Visualization", 
                               font=('Arial', 16, 'bold'))
        title_label.pack()
    ...
    def add_step(self, array: List[int], current_element: int = -1, 
                 insertion_position: int = -1, comparing_index: int = -1,
                 sorted_indices: List[int] = None, shifting_indices: List[int] = None,
                 description: str = ""):
        """Add a step to the Insertion Sort visualization"""
        step_data = {
            'array': array.copy(),
            ...
            'shifting_indices': shifting_indices if shifting_indices else [],
    ...
    def sort_algorithm(self, arr: List[int]):
        ...
        # Add initial step
        self.add_step(arr, sorted_indices=[0], description="Initial state - First element is considered sorted")
        
        # Insertion sort main loop
        for i in range(1, n):
            key = arr[i]
            sorted_indices = list(range(i))  # Elements before i are sorted
            
            self.add_step(
                arr,
                current_element=i,
                sorted_indices=sorted_indices,
                description=f"Round {i}: Taking element {key} at position {i} to insert into sorted portion"
            )
            ...
            while j >= 0:
                self.add_step(
                    ...
                    comparing_index=j,
                    sorted_indices=sorted_indices,
                    description=f"Comparing: {key} with {arr[j]} at position {j}"
                )
                ...
                if arr[j] > key:
                    self.add_step(
                        ...
                        current_element=i,
                        comparing_index=j,
                        shifting_indices=[j, j + 1],
                        sorted_indices=sorted_indices,
                        description=f"Shifting: {arr[j]} from position {j} to position {j + 1}"
                    )
                    ...
                    self.add_step(
                        ...
                        current_element=i,
                        sorted_indices=sorted_indices,
                        description=f"Shifted: {arr[j + 1]} is now at position {j + 1}"
                    )
                    ...
                    self.add_step(
                        ...
                        comparing_index=j,
                        sorted_indices=sorted_indices,
                        description=f"Found correct position: {key} should be placed after {arr[j]}"
                    )
                    break
            ...
            # Update sorted indices to include the newly inserted element
            new_sorted_indices = list(range(i + 1))
            
            self.add_step(
                arr,
                insertion_position=j + 1,
                sorted_indices=new_sorted_indices,
                description=f"Inserted: {key} at position {j + 1}. Sorted portion now: [0:{i}]"
            )
    ...
    def draw_array(self, step_data: Dict = None):
        ...
        if step_data:
            ...
            # Color current element being inserted (blue)
            current_elem = step_data.get('current_element', -1)
            if 0 <= current_elem < len(colors):
                colors[current_elem] = self.colors['current']
            ...
            # Color shifting elements (purple)
            for idx in step_data.get('shifting_indices', []):
                if 0 <= idx < len(colors):
                    colors[idx] = self.colors['pivot']  # Using pivot color for shifting
            
            # Color insertion position (red)
            insert_pos = step_data.get('insertion_position', -1)
            if 0 <= insert_pos < len(colors):
                colors[insert_pos] = self.colors['minimum']  # Using minimum color for insertion
        ...
        legend_elements = [
            ...
            Patch(facecolor=self.colors['current'], label='Current Element'),
            Patch(facecolor=self.colors['comparing'], label='Comparing'),
            Patch(facecolor=self.colors['pivot'], label='Shifting'),
            Patch(facecolor=self.colors['minimum'], label='Insertion Position'),
    ...
    def start_sorting(self):
        ...
        self.log_message("Insertion Sort started...")
    ...
    def fast_sort(self):
        """Perform fast Insertion Sort with quick animation steps"""
        if self.is_sorting:
            return
    ...
    def pause_sorting(self):
        """Pause the Insertion Sort animation"""
        ...
        self.log_message("Insertion Sort paused.")
    
    def reset_sorting(self):
        """Reset the Insertion Sort animation"""
        super().reset_sorting()
        ...
        self.log_message("Insertion Sort reset.")
    ...
    def animate_sorting(self):
        """Animate the Insertion Sort process"""
        if not self.is_playing or self.current_step >= len(self.steps):
            if self.current_step >= len(self.steps):
                ...
                if hasattr(self, 'original_speed'):
                    ...
                    self.log_message("Insertion Sort completed!")
