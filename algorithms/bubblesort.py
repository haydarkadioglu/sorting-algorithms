...
Complete visualization of the Bubble Sort algorithm with step-by-step animation.
"""
...
class BubbleSortAlgorithm(SortingAlgorithmBase):
    """
    Bubble Sort algorithm visualization implementation
    """
    
    # Algorithm metadata
    algorithm_name = "Bubble Sort"
    description = """Bubble Sort is a simple comparison-based sorting algorithm. It repeatedly steps through 
    the list, compares adjacent elements and swaps them if they are in the wrong order. The pass through 
    the list is repeated until the list is sorted. The algorithm gets its name from the way smaller 
    ...
    def create_gui(self):
        """Create the Bubble Sort specific GUI components"""
        ...
        title_label = ttk.Label(title_frame, text="Bubble Sort Algorithm Visualization", 
                               font=('Arial', 16, 'bold'))
        title_label.pack()
    ...
    def add_step(self, array: List[int], comparing_indices: List[int] = None, 
                 swapped_indices: List[int] = None, sorted_indices: List[int] = None,
                 description: str = ""):
        """Add a step to the Bubble Sort visualization"""
        step_data = {
            'array': array.copy(),
            'comparing_indices': comparing_indices if comparing_indices else [],
            'swapped_indices': swapped_indices if swapped_indices else [],
    ...
    def sort_algorithm(self, arr: List[int]):
        ...
        # Add initial step
        self.add_step(arr, description="Initial state - Bubble Sort algorithm starting")
        ...
        for i in range(n):
            ...
            self.add_step(
                arr, 
                description=f"Pass {i + 1}: Bubbling largest unsorted element to position"
            )
            ...
            for j in range(0, n - i - 1):
                ...
                if arr[j] > arr[j + 1]:
                    self.add_step(
                        ...
                        sorted_indices=sorted_indices.copy(),
                        description=f"Swapping: {arr[j]} > {arr[j + 1]}"
                    )
                    ...
                    self.add_step(
                        arr,
                        sorted_indices=sorted_indices.copy(),
                        description=f"Swap completed: {arr[j]} and {arr[j + 1]}"
                    )
            ...
            self.add_step(
                arr,
                sorted_indices=sorted_indices.copy(),
                description=f"Pass {i + 1} completed. Element {arr[n - i - 1]} is now in correct position"
            )
            ...
            if not swapped:
                ...
                self.add_step(
                    arr,
                    sorted_indices=sorted_indices.copy(),
                    description="No swaps needed. Array is now sorted!"
                )
                break
    ...
    def draw_array(self, step_data: Dict = None):
        ...
        if step_data:
            ...
            for idx in step_data.get('comparing_indices', []):
                if 0 <= idx < len(colors) and colors[idx] != self.colors['sorted']:
                    colors[idx] = self.colors['comparing']
            
            # Color swapped elements (override comparing color)
            for idx in step_data.get('swapped_indices', []):
                if 0 <= idx < len(colors) and colors[idx] != self.colors['sorted']:
                    colors[idx] = self.colors['current']
        ...
        legend_elements = [
            ...
            Patch(facecolor=self.colors['current'], label='Swapping'),
    ...
    def start_sorting(self):
        ...
        self.log_message("Bubble Sort started...")
    ...
    def fast_sort(self):
        """Perform fast Bubble Sort with quick animation steps"""
        if self.is_sorting:
            return
    ...
    def pause_sorting(self):
        """Pause the Bubble Sort animation"""
        ...
        self.log_message("Bubble Sort paused.")
    
    def reset_sorting(self):
        """Reset the Bubble Sort animation"""
        super().reset_sorting()
        ...
        self.log_message("Bubble Sort reset.")
    ...
    def animate_sorting(self):
        """Animate the Bubble Sort process"""
        if not self.is_playing or self.current_step >= len(self.steps):
            if self.current_step >= len(self.steps):
                ...
                if hasattr(self, 'original_speed'):
                    ...
                    self.log_message("Bubble Sort completed!")
